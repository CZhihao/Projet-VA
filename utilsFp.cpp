...
filtreParticulaire::init(double pose[3], int nbParticule)
{
...
void utils::initScanLidar(xmlConfig * _config)
{
	...
	catch(cv::Exception e)
	{
		...
		s+=e.what(); //Est censé fournir un message explicite sur le pourquoi du comment de l’exception.
		MAPS::ReportError(s);
		_bIsMapLoaded = false;
	}
}
...
double utils::rayTracing(int xPix, int yPix, double Alpha)
{
    ...
    // Calcul de l'extrémité du rayon
    CvPoint extremite;
...
void utils::lidarSim(double pose[3], vector<double> &Distances)
{
	...
	// todo tenir compte du déplacement du robot qui s'ajoute à rotation du lidar
	double xPix,yPix;
